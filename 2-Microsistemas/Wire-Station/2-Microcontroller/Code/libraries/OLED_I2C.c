...
#define OLED_I2C_ADDRESS 0x3C //� 0x7A  direcci�n I2C
...
void OLED_Display(void)
{
   ...
   { 
      ...
      i2c_start (OLED_stream); // inicia la comunicaci�n
      i2c_write (OLED_stream,OLED_I2C_ADDRESS); //direccion I2C             
      i2c_write (OLED_stream,0x40); //Control de direcci�n
      for (uint8_t i=1; i<=OLED_LCDWIDTH; ++i)
      {      
         i2c_write (OLED_stream,OLED_buffer[n2]);      
         n2++;
      }
       i2c_stop (OLED_stream); //termina la comunicaci�n
   }
}
